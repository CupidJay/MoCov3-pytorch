...
def main():
    ...
    args.save_dir = os.path.join(args.save_dir, 'self-similarity', args.arch)
    if args.pretrained:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        moco.loader.TwoScaleCropsTransform(transforms.Compose(augmentation)))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        #else:
        #    output, target = model(im_q=images[1], im_k=images[0])
